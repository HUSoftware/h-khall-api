...
public class ScheduleSql extends AbstractSql implements ScheduleDao
{
  ...
  {
    mStmts = getStatements("Schedule.json");

    Statement upsertYM = mStmts.getStatement("UPSERT_YM");
    mUpsertYM = newSqlUpdate(inDataSource, upsertYM.getSql(), upsertYM.gTypes());

    Statement upsertYMP = mStmts.getStatement("UPSERT_YMP");
    mUpsertYMP = newSqlUpdate(inDataSource, upsertYMP.getSql(), upsertYMP.gTypes());

    Statement update = mStmts.getStatement("UPDATE");
    mUpdate = newSqlUpdate(inDataSource, update.getSql(), update.gTypes());

    Statement selectY = mStmts.getStatement("SELECT_Y");
    mSelectY = new MappingSql<Assignment>(inDataSource, selectY.getSql(), selectY.gTypes())
    {
      @Override
    ...
    mSelectYM = new MappingSql<Assignment>(inDataSource, selectYM.getSql(), selectYM.gTypes())
    {
      @Override
      public Assignment mapRow(ResultSet inRs, int inRowNum) throws SQLException
      {
    ...
    mSelectByYM = newQuery(inDataSource, selectYM.getSql(), selectYM.gTypes());
  }
